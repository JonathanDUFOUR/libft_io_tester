...
/*   Updated: 2021/11/27 01:18:32 by jodufour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	test_valid(int const i, int const *fd, int *const ret)
{
	...
	if (ft_putlluint_base_fd(g_test[i].nb, g_test[i].base, fd[1]) == -1)
		return (*ret = FT_PUTLLUINT_BASE_FD_ERR);
...
static int	test_invalid(int const i, int const *fd, int *const ret)
{
	...
	if (write(fd[1], "", 1) == -1)
		return (WRITE_ERR);
	ft_ret = ft_putlluint_base_fd(g_test[i].nb, g_test[i].base, fd[1]);

...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/16 23:44:54 by jodufour          #+#    #+#             */
/*   Updated: 2021/11/27 01:05:16 by jodufour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{6, 69, "ABBA", NULL},
{7, 42, "01", "101010"},
{8, 12345, "o_O", "_O_OO_oOo"},
{9, SHRT_MAX, "poneyvif", "fffff"},
{10, 42101, "0123456789abcdef", "a475"},
{11, USHRT_MAX, "@#$", "#@@$$$$@@$@"},
{12, 0, "fruit", "f"},
{0}
};
...
static int	test_valid(int const i, int const *fd, int *const ret)
{
	...
	if (ft_puthuint_base_fd(g_test[i].nb, g_test[i].base, fd[1]) == -1)
		return (*ret = FT_PUTHUINT_BASE_FD_ERR);
...
static int	test_invalid(int const i, int const *fd, int *const ret)
{
	...
	if (write(fd[1], "", 1) == -1)
		return (WRITE_ERR);
	ft_ret = ft_puthuint_base_fd(g_test[i].nb, g_test[i].base, fd[1]);

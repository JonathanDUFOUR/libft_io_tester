...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/16 23:38:00 by jodufour          #+#    #+#             */
/*   Updated: 2021/11/27 00:57:50 by jodufour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	test_valid(int const i, int const *fd, int *const ret)
{
	...
	if (ft_puthhuint_base_fd(g_test[i].nb, g_test[i].base, fd[1]) == -1)
		return (*ret = FT_PUTHHUINT_BASE_FD_ERR);
...
static int	test_invalid(int const i, int const *fd, int *const ret)
{
	...
	if (write(fd[1], "", 1) == -1)
		return (WRITE_ERR);
	ft_ret = ft_puthhuint_base_fd(g_test[i].nb, g_test[i].base, fd[1]);

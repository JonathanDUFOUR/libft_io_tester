...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/16 23:43:27 by jodufour          #+#    #+#             */
/*   Updated: 2022/04/27 09:31:43 by jodufour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{5, 89, "f\t", NULL},
{6, -69, "ABBA", NULL},
{7, 4242, "01", "1000010010010"},
{8, -8421, "o_O", "-_oO__OOOo"},
{9, SHRT_MAX, "poneyvif", "fffff"},
{10, SHRT_MIN, "0123456789abcdef", "-8000"},
{11, (t_hint)USHRT_MAX, "@#$", "-#"},
{12, 0, "fruit", "f"},
{0}
};
...
static int	test_valid(int const i, int const *fd, int *const ret)
{
	...
	if (ft_puthint_base_fd(g_test[i].nb, g_test[i].base, fd[1]) == -1)
		return (*ret = FT_PUTHINT_BASE_FD_ERR);
...
static int	test_invalid(int const i, int const *fd, int *const ret)
{
	...
	if (write(fd[1], "", 1) == -1)
		return (WRITE_ERR);
	ft_ret = ft_puthint_base_fd(g_test[i].nb, g_test[i].base, fd[1]);

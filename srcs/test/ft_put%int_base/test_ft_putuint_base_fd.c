...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/17 00:58:47 by jodufour          #+#    #+#             */
/*   Updated: 2022/04/27 09:32:22 by jodufour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{10, 987654321, "0123456789abcdef", "3ade68b1"},
{11, UINT_MAX, "@#$", "#@$@@$@$$$@#$$####$#@"},
{12, 0, "fruit", "f"},
{0}
};
...
static int	test_valid(int const i, int const *fd, int *const ret)
{
	...
	if (ft_putuint_base_fd(g_test[i].nb, g_test[i].base, fd[1]) == -1)
		return (*ret = FT_PUTUINT_BASE_FD_ERR);
...
static int	test_invalid(int const i, int const *fd, int *const ret)
{
	...
	if (write(fd[1], "", 1) == -1)
		return (WRITE_ERR);
	ft_ret = ft_putuint_base_fd(g_test[i].nb, g_test[i].base, fd[1]);

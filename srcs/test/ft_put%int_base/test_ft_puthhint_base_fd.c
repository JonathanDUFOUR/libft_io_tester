...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/16 23:36:40 by jodufour          #+#    #+#             */
/*   Updated: 2021/11/27 00:42:20 by jodufour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{7, 42, "01", "101010"},
{8, -88, "o_O", "-_ooO_"},
{9, CHAR_MAX, "poneyvif", "off"},
{10, CHAR_MIN, "0123456789abcdef", "-80"},
{11, UCHAR_MAX, "@#$", "-#"},
{12, 0, "fruit", "f"},
{0}
};
...
static int	test_valid(int const i, int const *fd, int *const ret)
{
	...
	if (ft_puthhint_base_fd(g_test[i].nb, g_test[i].base, fd[1]) == -1)
		return (*ret = FT_PUTHHINT_BASE_FD_ERR);
...
static int	test_invalid(int const i, int const *fd, int *const ret)
{
	...
	if (write(fd[1], "", 1) == -1)
		return (WRITE_ERR);
	ft_ret = ft_puthhint_base_fd(g_test[i].nb, g_test[i].base, fd[1]);
